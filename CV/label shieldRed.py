...
def getContour(img):
    ...
    lower = np.array([79, 1, 250])
    upper = np.array([131, 17, 255])
    ...
    try:
        ...
        point1 = [rect1[0]+widthArr[point[0]]/2,rect1[1]-20]
        point2 = [rect1[0]+widthArr[point[0]]/2,rect1[1]+lengthArr[point[0]]+20]
        point3 = [rect2[0]+widthArr[point[1]]/2,rect2[1]-20]
        point4 = [rect2[0]+widthArr[point[1]]/2,rect2[1]+lengthArr[point[1]]+20]
        if (point1[0] > point2[0] and point1[1] != point4[1] and find_number):
        ...
        rvec, tvec = pnpSolver(pnpIn, True)
        print(tvec)
...
cap = cv2.VideoCapture("test sample/test1.avi")
while True:
    success, img = cap.read()
    #img = cv2.imread()
